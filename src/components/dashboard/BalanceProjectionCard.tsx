...
import { fetchWidget, type WidgetName } from "../../lib/dashboardApi";
...
function getTreasuryView(daysRange: number): ViewConfig {
  if (daysRange <= 30) {
    return { widgetName: "treasury_client_totals", periodField: "snapshot_date", dateField: "snapshot_date", isMonthly: false };
  }
  if (daysRange <= 60) {
    return { widgetName: "treasury_weekly_client_totals", periodField: "week_start", dateField: "last_snapshot_date", isMonthly: false };
  }
  if (daysRange <= 180) {
    return { widgetName: "treasury_biweekly_client_totals", periodField: "period_start", dateField: "last_snapshot_date", isMonthly: false };
  }
  return { widgetName: "treasury_monthly_client_totals", periodField: "month", dateField: "last_snapshot_date", isMonthly: true };
}
...
export default function BalanceProjectionCard() {
  ...
  const selectedClientCode = selectedClient?.code ?? null;
  const daysRange = 30;
  const viewConfig = useMemo(() => getTreasuryView(daysRange), [daysRange]);

  const { data, isLoading, isError, error, isFetching } = useQuery({
    queryKey: ["treasury-history", selectedClientCode, viewConfig.widgetName],
    queryFn: () => fetchWidget<TreasuryRow>(viewConfig.widgetName, selectedClientCode),
    enabled: !!selectedClientCode && !clientsLoading && !clientsError,
    staleTime: 60_000,
  ...
  const series: ChartPoint[] = useMemo(() => {
    ...
    return data.map((row) => {
      ...
      if (viewConfig.isMonthly) {
        ...
        label = monthDate.toLocaleDateString("es-ES", { month: "short", year: "2-digit" });
      } else {
        const periodStr = row[viewConfig.periodField] as string;
        const periodDate = new Date(periodStr);
        label = periodDate.toLocaleDateString("es-ES", { day: "2-digit", month: "2-digit" });
      }
  ...
  if (clientsError) {
    return (
      <Card><CardHeader><CardTitle>{title}</CardTitle><CardDescription>No se ha podido cargar la información.</CardDescription></CardHeader>
        <CardContent><Alert variant="destructive"><AlertDescription>{clientsError}</AlertDescription></Alert></CardContent></Card>
    );
  }

  if (clientsLoading || !selectedClientId || !selectedClient) {
    return (
      <Card><CardHeader><CardTitle>{title}</CardTitle><CardDescription>Cargando datos...</CardDescription></CardHeader>
        <CardContent className="space-y-3"><Skeleton className="h-6 w-32" /><Skeleton className="h-40 w-full" /></CardContent></Card>
    );
  }

  if (isError) {
    return (
      <Card><CardHeader><CardTitle>{title}</CardTitle><CardDescription>No se ha podido cargar el historial.</CardDescription></CardHeader>
        <CardContent><Alert variant="destructive"><AlertDescription>{(error as Error)?.message || "Error al recuperar los datos."}</AlertDescription></Alert></CardContent></Card>
    );
  }

  if (isLoading) {
    return (
      <Card><CardHeader><CardTitle>{title}</CardTitle><CardDescription>Consultando historial...</CardDescription></CardHeader>
        <CardContent className="space-y-3"><Skeleton className="h-6 w-32" /><Skeleton className="h-40 w-full" /></CardContent></Card>
    );
  }
  ...
  if (!data || data.length === 0) {
    return (
      <Card><CardHeader><CardTitle>{title}</CardTitle><CardDescription>No hay datos históricos disponibles.</CardDescription></CardHeader>
        <CardContent><p className="text-sm text-muted-foreground">Aún no hay registros de tesorería.</p></CardContent></Card>
    );
  }
  ...
  return (
    <Card className="font-sans">
      ...
      <CardContent className="space-y-6">
        ...
        {chartFooterText && (
          <p className="text-[10px] text-muted-foreground/60">{chartFooterText}</p>
        )}
