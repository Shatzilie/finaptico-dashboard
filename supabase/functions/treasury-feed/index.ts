...
serve(async (req) => {
  ...
  const clientCode = reqUrl.searchParams.get("client_code") ?? null;
  ...
  try {
    ...
    const result = await pgClient.queryObject(
      "SELECT DISTINCT ON (client_code, instance_code) client_code, instance_code, snapshot_date, total_balance, currency FROM erp_core.v_treasury_client_totals WHERE client_code = ANY($1) ORDER BY client_code, instance_code, snapshot_date DESC",
      [targets]
    );

    console.log("[treasury-feed] rows:", result.rows.length, "user:", userId, "clients:", targets.join(","));
